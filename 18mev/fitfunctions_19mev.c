...
Double_t gaussianpeak(Double_t *x, Double_t *par) {
	...
	if (par[0]) {
	  ...
	  arg2 = (x[0] - par[1] + 26.0)/par[0];
	  arg3 = (x[0] - par[1] + 31.6)/par[0];
	  arg4 = (x[0] - par[1] + 42.4)/par[0];
	  arg5 = (x[0] - par[1] + 59.3)/par[0];
...
int fit0D(int angle, TFile* f2, FILE* f3, TFile* fFit){
	...
	gasout1_1clone->Scale(2.17); //product of ratio of gas in to gas out bcis and ratio of gas in to gas out live times

	f2->cd();

	/* Fit range parameters */

	Int_t lu1 = 2608; //from 2618 - 10
	Int_t hu1 = 2827; //from 2677 + 150
...
//********************************************************************
	...
	fitfunc2->SetParameter(0,213.5);  		//constant
	fitfunc2->SetParameter(1,-0.07432);  		//linear
	fitfunc2->FixParameter(2,0.0);  		//quad
	fitfunc2->SetParameter(3,9.634);  	//sigma
	fitfunc2->SetParameter(4,2678.0);  //mean
	fitfunc2->SetParameter(5,321.0);   //amp1
	...
	fitfunc2->SetParameter(6,0.1261);     //amp2   2+ states
	...
	fitfunc2->SetParameter(8,0.0145);     //amp4   2+ states
	fitfunc2->SetParameter(9,0.108);    	//amp5  50->0.1
...
int fit3D(int angle, TFile* f2, FILE* f3, TFile* fFit){
	...
	Int_t lu1 = 2556; //2561 - 5
	Int_t hu1 = 2770; //2620 + 150
	...
	TF1 *fitfunc2 = new TF1("fitfunc2", fitfuncc, lu1, hu1, 12);
	fitfunc2->SetParameter(0,77.48);  		//constant
	fitfunc2->SetParameter(1,-0.02692);  		//linear
	fitfunc2->FixParameter(2,0);  		//quad
	fitfunc2->SetParameter(3,9.978);  	//sigma
	fitfunc2->SetParameter(4,2620.0);  //mean
	fitfunc2->SetParameter(5,243.5);   //amp1
	fitfunc2->SetParameter(6,0.07245);     //amp2   2+ states
	fitfunc2->FixParameter(7,0.0);    	//amp3  50->0.1
	fitfunc2->SetParameter(8,0.01477);     //amp4   2+ states
	fitfunc2->SetParameter(9,0.1197);    	//amp5  50->0.1
	fitfunc2->FixParameter(10,0.0);			//amp6
	fitfunc2->FixParameter(11,0.0);			//amp7
...
int fit6D(int angle, TFile* f2, FILE* f3, TFile* fFit){
	...
	Int_t lu1 = 2572; //2577 - 5
	Int_t hu1 = 2786; //150 + 2636
	...
	fitfunc2->SetParameter(4,2636.0);  //mean
...
int fit9D(int angle, TFile* f2, FILE* f3, TFile* fFit){
		...
		Int_t lu1 = 2570; //2575 - 5
		Int_t hu1 = 2784; //2634 + 150
		...
		fitfunc2->SetParameter(4,2634.0);  //mean
...
int fit12D(int angle, TFile* f2, FILE* f3, TFile* fFit){
		...
		Int_t lu1 = 2607; //2612 - 5
		Int_t hu1 = 2800; //2671 + 150
		...
		fitfunc2->SetParameter(4,2671.0);  //mean
...
int fit15D(int angle, TFile* f2, FILE* f3, TFile* fFit){
		...
		Int_t lu1 = 2593; //2598 - 5
		Int_t hu1 = 2806; //2656 + 150
		...
		fitfunc2->SetParameter(4,2656.0);  //mean
...
int fit18D(int angle, TFile* f2, FILE* f3, TFile* fFit){
		...
		Int_t lu1 = 2574; //2579 - 5
		Int_t hu1 = 2789; //2639 + 150
		...
		fitfunc2->SetParameter(4,2638.0);  //mean
